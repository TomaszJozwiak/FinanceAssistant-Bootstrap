	...
	if (!isset($_SESSION['loged_in']))
	{
		...
		if (isset($_POST['amount']) & isset($_POST['date']) & isset($_POST['method']) & isset($_POST['category']))
		{
			...
			try 
			{
				...
				{
					$category_id_query = $connection->query("SELECT id FROM expenses_category_assigned_to_users WHERE user_id='$user_ID' AND name='$posted_category'");
					$category_id_row=$category_id_query->fetch_assoc();
					$category_id=$category_id_row['id'];
					
					$method_id_query = $connection->query("SELECT id FROM payment_methods_assigned_to_users WHERE user_id='$user_ID' AND name='$posted_method'");
					$method_id_row=$method_id_query->fetch_assoc();
					$method_id=$method_id_row['id'];
					
					if (!$category_id_query) throw new Exception($connection->error);
					if (!$method_id_query) throw new Exception($connection->error);
					
					if (isset($_POST['comment'])){
						if ($connection->query("INSERT INTO expenses VALUES (NULL, '$user_ID', '$category_id', '$method_id', '$amount', '$date', '$comment')"))
						$_SESSION['successful_expense_addition']=true;
					}
					
					else{
						if ($connection->query("INSERT INTO expenses VALUES (NULL, '$user_ID', '$category_id', '$method_id', '$amount', '$date', NULL)"))
						$_SESSION['successful_expense_addition']=true;
					}
				}
			}
				...
				<div class="row text-center">
					<div class="col-sm-6 col-sm-offset-3"> 
						<article>
							...
							<?php					
								if (isset($_SESSION['successful_expense_addition']))
								{
										echo '<div style="color: limegreen;">'.'Gratulacje, wydatek został dodany'.'</div>';
										unset($_SESSION['successful_expense_addition']);
								}
							?>			
							</br>
							<form method="post">
								...
								<div class="form-group">
									...
									try 
									{
										...
										{
											$user_ID=$_SESSION['logged_user_ID'];
											$method_name_query=$connection->query("SELECT name FROM payment_methods_assigned_to_users WHERE user_id='$user_ID'");
								...
								<div class="form-group">
									...
									try 
									{
										...
										{
											$category_name_query=$connection->query("SELECT name FROM expenses_category_assigned_to_users WHERE user_id='$user_ID'");
